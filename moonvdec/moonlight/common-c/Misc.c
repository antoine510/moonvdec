...
int isReferenceFrameInvalidationEnabled(PLIMELIGHT_CTX ctx) {
    LC_ASSERT(ctx->NegotiatedVideoFormat != 0);
    return ((ctx->NegotiatedVideoFormat & VIDEO_FORMAT_MASK_H264) && (ctx->VideoCallbacks.capabilities & CAPABILITY_REFERENCE_FRAME_INVALIDATION_AVC)) ||
           ((ctx->NegotiatedVideoFormat & VIDEO_FORMAT_MASK_H265) && (ctx->VideoCallbacks.capabilities & CAPABILITY_REFERENCE_FRAME_INVALIDATION_HEVC));
}
