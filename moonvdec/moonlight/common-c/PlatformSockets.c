...
void setRecvTimeout(PLIMELIGHT_CTX ctx, SOCKET s, int timeoutSec) {
#if defined(LC_WINDOWS)
...
SOCKET bindUdpSocket(PLIMELIGHT_CTX ctx, int addrfamily, int bufferSize) {
    SOCKET s;
...
SOCKET connectTcpSocket(PLIMELIGHT_CTX ctx, struct sockaddr_storage* dstaddr, SOCKADDR_LEN addrlen, unsigned short port, int timeoutSec) {
    SOCKET s;
...
#ifdef FIONBIO
    {
        ...
        err = select((int)s + 1, NULL, &writefds, &exceptfds, &tv);
        if (err < 0) {
            // select() failed
            err = LastSocketError();
...
int resolveHostName(PLIMELIGHT_CTX ctx, const char* host, struct sockaddr_storage* addr, SOCKADDR_LEN* addrLen)
{
#ifndef __vita__
    ...
    *addrLen = (SOCKADDR_LEN)res->ai_addrlen;
