...
static int reconstructFrame(PLIMELIGHT_CTX ctx, PRTP_FEC_QUEUE queue) {
    ...
    int receiveSize = ctx->StreamConfig.packetSize + MAX_RTP_HEADER_SIZE;
    ...
    for (i = 0; i < totalPackets; i++) {
        if (marks[i]) {
            ...
            if (ret == 0 && i < queue->bufferDataPackets) {
                ...
                queuePacket(queue, queueEntry, 0, rtpPacket, ctx->StreamConfig.packetSize + dataOffset, 0);
...
int RtpfAddPacket(PLIMELIGHT_CTX ctx, PRTP_FEC_QUEUE queue, PRTP_PACKET packet, int length, PRTPFEC_QUEUE_ENTRY packetEntry) {
    ...
    if (!queuePacket(queue, packetEntry, 0, packet, length, !isBefore16(packet->sequenceNumber, queue->bufferFirstParitySequenceNumber))) {
        ...
        if (reconstructFrame(ctx, queue) == 0) {
