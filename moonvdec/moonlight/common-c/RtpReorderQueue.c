...
static PRTP_QUEUE_ENTRY enforceQueueConstraints(PLIMELIGHT_CTX ctx, PRTP_REORDER_QUEUE queue) {
    int dequeuePacket = 0;
...
int RtpqAddPacket(PLIMELIGHT_CTX ctx, PRTP_REORDER_QUEUE queue, PRTP_PACKET packet, PRTP_QUEUE_ENTRY packetEntry) {
    ...
    if (queue->queueHead == NULL) {
        ...
        lowestEntry = enforceQueueConstraints(ctx, queue);
