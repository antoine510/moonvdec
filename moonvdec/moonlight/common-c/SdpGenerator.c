...
static int addGen5Options(PLIMELIGHT_CTX ctx, PSDP_OPTION* head) {
    int err = 0;
    ...
    if (ctx->StreamConfig.width >= 3840 && ctx->StreamConfig.height >= 2160) {
...
static PSDP_OPTION getAttributesList(PLIMELIGHT_CTX ctx, char*urlSafeAddr) {
    ...
    if (ctx->AppVersionQuad[0] >= 5) {
        ...
        if (ctx->StreamConfig.width * ctx->StreamConfig.height <= 1366 * 768) {
            // 720p
            if (ctx->StreamConfig.fps <= 30) {
                // 30 FPS
        ...
        else if (ctx->StreamConfig.width * ctx->StreamConfig.height <= 1920 * 1200) {
            // 1080p
            if (ctx->StreamConfig.fps <= 30) {
                // 30 FPS
        ...
        else if (ctx->StreamConfig.width * ctx->StreamConfig.height <= 2560 * 1600) {
            // 1440p
            if (ctx->StreamConfig.fps <= 30) {
                // 30 FPS
        ...
        sprintf(payloadStr, "%d", maxEncodingBitrate < ctx->StreamConfig.bitrate ?
                                  maxEncodingBitrate : ctx->StreamConfig.bitrate);
        ...
        sprintf(payloadStr, "%d", ctx->StreamConfig.bitrate);
        ...
        if (ctx->StreamConfig.streamingRemotely) {
    ...
    if (ctx->AppVersionQuad[0] >= 4) {
        unsigned char slicesPerFrame;

        // Use slicing for increased performance on some decoders
        slicesPerFrame = (unsigned char)(ctx->VideoCallbacks.capabilities >> 24);
        if (slicesPerFrame == 0) {
        ...
        if (ctx->NegotiatedVideoFormat & VIDEO_FORMAT_MASK_H265) {
            ...
            if (ctx->AppVersionQuad[0] < 7 ||
               ...
               (ctx->AppVersionQuad[0] == 7 && ctx->AppVersionQuad[1] == 1 && ctx->AppVersionQuad[2] < 408)) {
        ...
        if (ctx->AppVersionQuad[0] >= 7) {
            ...
            sprintf(payloadStr, "%d", ctx->StreamConfig.clientRefreshRateX100);
            err |= addAttributeString(&optionHead, "x-nv-video[0].clientRefreshRateX100", payloadStr);
        }

        if (ctx->StreamConfig.audioConfiguration == AUDIO_CONFIGURATION_51_SURROUND) {
        ...
        if (ctx->AppVersionQuad[0] >= 7) {
            ...
            if (ctx->OriginalVideoBitrate >= HIGH_BITRATE_THRESHOLD && audioChannelCount > 2) {
...
static int fillSdpHeader(PLIMELIGHT_CTX ctx, char* buffer, int rtspClientVersion, char*urlSafeAddr) {
    return sprintf(buffer,
        "v=0\r\n"
        "o=android 0 %d IN %s %s\r\n"
        "s=NVIDIA Streaming Client\r\n",
        rtspClientVersion,
		ctx->RemoteAddr.ss_family == AF_INET ? "IPv4" : "IPv6",
        urlSafeAddr);
}

// Populate the SDP tail with required information
static int fillSdpTail(PLIMELIGHT_CTX ctx, char* buffer) {
    return sprintf(buffer,
        "t=0 0\r\n"
        "m=video %d  \r\n",
		ctx->AppVersionQuad[0] < 4 ? 47996 : 47998);
}
...
char* getSdpPayloadForStreamConfig(PLIMELIGHT_CTX ctx, int rtspClientVersion, int* length) {
    ...
    addrToUrlSafeString(&ctx->RemoteAddr, urlSafeAddr);
    ...
    offset = fillSdpHeader(ctx, payload, rtspClientVersion, urlSafeAddr);
    offset += fillSerializedAttributeList(&payload[offset], attributeList);
    offset += fillSdpTail(ctx, &payload[offset]);
